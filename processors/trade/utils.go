...
func genCAViewQuery(view, table, period string) string {
	return fmt.Sprintf(`CREATE MATERIALIZED VIEW %v WITH (timescaledb.continuous) AS select time_bucket('%v', time) AS period, first(price,time) open, last(price,time) "close",avg(price) as Average,max(price) as high,min(price) as low, sum(price) as volume, count(*) as "set" from %v GROUP BY period WITH NO DATA`, view, period, table)
}
